    ...
    if (!hasInitializedKernels) {
        ...
        for (int step = 0; step < numSteps; step++) {
            if ((stepType[step] == CustomIntegrator::ComputePerDof || stepType[step] == CustomIntegrator::ComputeSum) && !merged[step]) {
                ...
                cout << "customIntegratorPerDof:" << endl << program.getInfo(CL_PROGRAM_SOURCE) << endl; // DEBUG JDC
                ...
                cout << "customIntegrator:" << endl << program.getInfo(CL_PROGRAM_SOURCE) << endl; // DEBUG JDC
